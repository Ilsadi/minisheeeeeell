...
/*   Updated: 2025/09/24 15:57:36 by ilsadi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exit_with_rb(t_mini *mini, int code)
{
	rb_free_all(mini->rb);
	free(mini->rb);
	exit(code);
...
static void	child_prepare_io(t_pipex *p, t_token *tokens, t_mini *mini)
{
	if (handle_redirections(tokens, PIPE) < 0)
		exit_with_rb(mini, 1);
